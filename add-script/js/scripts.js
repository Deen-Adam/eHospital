(function ($) {
	...
	$(document).ready(function () {
		...
		/*==========  Slide Out  ==========*/
		$('.fbtn-alt').on('click', function(event) {
			event.preventDefault();
	...
	$('.contact-form').on('submit', function() {
		...
		if (validateEmail(contactForm.find('.contact-email').val()) && contactForm.find('.contact-email').val().length !== 0 && contactForm.find('.contact-name').val().length !== 0 && contactForm.find('.contact-message').val().length !== 0) {
			...
			$.ajax({
				...
				success: function() {
					contactForm.find('.contact-loading').fadeOut();
					contactForm.find('.contact-success').find('.message').html('Success! Thanks for contacting Deenysoft!');
	...
	function initialize_map() {
		if ($('.map').length) {
			var myLatLng = new google.maps.LatLng(3.1486623, 101.5750858);
			var mapOptions = {
				zoom: 10,
				center: myLatLng,
				scrollwheel: false,
